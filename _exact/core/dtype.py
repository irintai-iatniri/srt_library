...
fixed32 = DType("fixed32", 4, rust_type="f32")
fixed64 = DType("fixed64", 8, rust_type="f64")
...
def get_dtype(dtype_spec: Union[DType, str, Any]) -> DType:
    ...
    if hasattr(dtype_spec, "itemsize"):
        ...
        if is_complex:
            ...
            return fixed64 if size >= 8 else fixed32
...
def promote_dtypes(dtype1: DType, dtype2: DType) -> DType:
    ...
    2. Exact types DO NOT auto-promote to fixed (prevents drift)
    3. fixed64 > fixed32 (fixed types promote as usual)
    4. Complex takes precedence over real (existing behavior)

    Critical Rule: Mixing exact and fixed types will raise an error.
    ...
    Raises:
        ValueError: If mixing exact and fixed types without explicit conversion
    """
    ...
    float_types = {"fixed64": 2, "fixed32": 1}
    ...
    # CRITICAL: Do not mix exact and fixed types
    ...
    # Both fixed - promote to higher precision fixed
...
def is_float_dtype(dtype: DType) -> bool:
    ...
    Returns:
        True if dtype is fixed (fixed32, fixed64)
    """
    return dtype.name in ("fixed32", "fixed64")
