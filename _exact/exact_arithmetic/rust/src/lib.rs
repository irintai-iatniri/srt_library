...
/// Get the golden ratio φ = (1 + √5) / 2 - exact representation
#[pyfunction]
fn srt_phi() -> GoldenExact {
    GoldenExact::phi()
}

/// Get the golden ratio inverse φ⁻¹ = φ - 1 - exact representation
#[pyfunction]
fn srt_phi_inv() -> GoldenExact {
    GoldenExact::phi_hat()
}

/// Get the q-deficit value q = universal syntony deficit - exact symbolic
#[pyfunction]
fn srt_q_deficit() -> PySymExpr {
    PySymExpr { inner: crate::exact::symexpr::SymExpr::q() }
}

/// Get π (pi) constant - exact symbolic
#[pyfunction]
fn srt_pi() -> PySymExpr {
    PySymExpr { inner: crate::exact::symexpr::SymExpr::pi() }
}

/// Get e (Euler's number) constant - exact symbolic
#[pyfunction]
fn srt_e() -> PySymExpr {
    PySymExpr { inner: crate::exact::symexpr::SymExpr::e() }
}
...
/// Square root: √x - symbolic when possible, numeric fallback
#[pyfunction]
fn srt_sqrt(x: PySymExpr) -> PySymExpr {
    // For now, create a power expression x^(1/2)
    // Future: implement symbolic square root simplification
    PySymExpr {
        inner: x.inner.pow(crate::exact::rational::Rational::new(1, 2))
    }
}

/// Exponential: e^x - exact symbolic
#[pyfunction]
fn srt_exp(x: PySymExpr) -> PySymExpr {
    PySymExpr {
        inner: x.inner.exp_of()
    }
}

/// Natural logarithm: ln(x) - exact symbolic
#[pyfunction]
fn srt_log(x: PySymExpr) -> PySymExpr {
    PySymExpr {
        inner: x.inner.ln_of()
    }
}
...
/// Sine: sin(x) - exact symbolic
#[pyfunction]
fn srt_sin(x: PySymExpr) -> PySymExpr {
    PySymExpr {
        inner: crate::exact::symexpr::SymExpr::Sin(Box::new(x.inner))
    }
}

/// Cosine: cos(x) - exact symbolic  
#[pyfunction]
fn srt_cos(x: PySymExpr) -> PySymExpr {
    PySymExpr {
        inner: crate::exact::symexpr::SymExpr::Cos(Box::new(x.inner))
    }
}
