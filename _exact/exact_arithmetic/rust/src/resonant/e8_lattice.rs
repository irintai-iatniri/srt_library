...
impl E8Lattice {
    pub fn new() -> Self {
        ...
        for i in 0..8 {
            for j in (i + 1)..8 {
                for s1 in [Rational::new(-1, 1), Rational::one()] {
                    for s2 in [Rational::new(-1, 1), Rational::one()] {
                        let mut v = [Rational::zero(); 8];
                        v[i] = s1;
                        v[j] = s2;
                        roots.push(v);
                    }
    ...
    /// Static helper to get roots as Vec<Vec<f64>> (boundary export method)
    pub fn generate_roots() -> Vec<Vec<f64>> {
        let lattice = Self::new();
        lattice.roots.into_iter().map(|arr| arr.iter().map(|r| r.to_f64()).collect()).collect()
    }
