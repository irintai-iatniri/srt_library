...
fn cpu_golden_batch_norm_1d(
    ...
    for feat_idx in 0..num_features {
        // Compute mean and variance across batch using exact arithmetic
        let mut sum_exact = crate::exact::Rational::zero();
        let mut sum_sq_exact = crate::exact::Rational::zero();
        let mut sum = 0.0;
        let mut sum_sq = 0.0;
        ...
        for batch_idx in 0..batch_size {
            ...
            let val_exact = crate::exact::Rational::from_f64_nearest(val, input.precision());
            sum_exact += val_exact;
            sum_sq_exact += val_exact * val_exact;
            sum += val;
            sum_sq += val * val;
        }

        let batch_size_rational = crate::exact::Rational::from_i64(batch_size as i64);
        let mean_exact = sum_exact / batch_size_rational;
        let variance_exact = (sum_sq_exact / batch_size_rational) - (mean_exact * mean_exact);
        let mean = mean_exact.to_f64();
        let variance = variance_exact.to_f64();
...
fn cpu_golden_batch_norm_2d(
    ...
    for chan_idx in 0..channels {
        ...
        let mut sum_sq = Rational::from_integer(0);
        let count = Rational::from_integer((batch_size * spatial_size) as i128);
        ...
        for batch_idx in 0..batch_size {
            for h in 0..height {
                for w in 0..width {
                    ...
                    let val = input_data[idx];
                    sum = sum.add(&Rational::from_f64_approx(val));
                    sum_sq = sum_sq.add(&Rational::from_f64_approx(val * val));
                }
            }
        }

        let mean = sum.div(&count);
        let variance = sum_sq.div(&count).sub(&mean.mul(&mean));

        // Normalize to zero mean, unit variance
        let std = variance.add(&Rational::from_f64_approx(eps)).sqrt().eval_f64();
        let scale = mode.scale_factor();

        // Get affine parameters
        let gamma = affine_gamma
            .map(|g| g.to_floats_core()[chan_idx])
            .unwrap_or(Rational::from_integer(1).eval_f64());

        let beta = affine_beta
            .map(|b| b.to_floats_core()[chan_idx])
            .unwrap_or(Rational::from_integer(0).eval_f64());
        ...
        for batch_idx in 0..batch_size {
            for h in 0..height {
                for w in 0..width {
                    ...
                    // Normalize: (x - mean) / std
                    let normalized = (val - mean.eval_f64()) / std;
...
mod tests {
    ...
    fn test_golden_batch_norm_1d_golden_mode() {
        ...
        let target_var = crate::constants::PHI_INV;
