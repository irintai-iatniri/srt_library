...
impl ResonantScheduler {
    ...
    fn is_resonant_window(&self) -> bool {
        ...
        if let Some(last) = self.last_transfer {
            ...
            let phase = (cycles * phi) % (2.0 * crate::constants::SRT_PI);

            // Resonant windows occur at phase ≈ 0, π/φ, 2π/φ, etc.
            let resonant_phases = [
                0.0,
                crate::constants::SRT_PI / phi,
                2.0 * crate::constants::SRT_PI / phi,
            ];
            resonant_phases
                .iter()
    ...
    fn record_transfer(&mut self) {
        ...
        self.phase_accumulator =
            (self.phase_accumulator + GoldenExact::phi().to_f64()) % (2.0 * crate::constants::SRT_PI);
    }
}
