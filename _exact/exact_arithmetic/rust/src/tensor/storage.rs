...
pub enum CpuData {
    ...
    // Exact arithmetic types (primary for _exact backend)
...
pub enum CudaData {
    ...
    // Exact arithmetic type for GPU (Q32.32 fixed-point)
...
impl TensorStorage {
        ...
        let cpu_data =
            match dtype {
                "float32" | "f32" | "float64" | "f64" => {
                    // Convert float inputs to exact types at the boundary
                    let values: Vec<GoldenExact> = data
                        .iter()
                        .map(|x| {
                            if let Ok(f) = x.extract::<f64>() {
                                Ok(GoldenExact::from_f64(f))
                            } else if let Ok(f) = x.extract::<f32>() {
                                Ok(GoldenExact::from_f64(f as f64))
                            } else {
                    ...
                    CpuData::GoldenExact(ArrayD::from_shape_vec(IxDyn(&shape), values).map_err(
    ...
    pub fn zeros(shape: Vec<usize>, dtype: &str, device: &str) -> PyResult<Self> {
        ...
        let cpu_data = match dtype {
            "float32" | "f32" | "float64" | "f64" => CpuData::GoldenExact(ArrayD::from_elem(dim, GoldenExact::zero())),
    ...
    // to_float64() removed from _exact backend - use exact types only
    ...
    pub fn abs(&self) -> PyResult<TensorStorage> {
        ...
        let cpu = self.ensure_cpu()?;
        let result = match cpu {
            CpuData::Complex128(arr) => CpuData::GoldenExact(arr.mapv(|x| GoldenExact::from_f64(x.norm()))),
            CpuData::Int64(arr) => CpuData::Int64(arr.mapv(|x| x.abs())),
            CpuData::GoldenExact(arr) => {
                // Abs of GoldenExact: stay in exact arithmetic
                CpuData::GoldenExact(arr.mapv(|x| x.abs()))
            }
            CpuData::Rational(arr) => {
    ...
    pub fn exp(&self) -> PyResult<TensorStorage> {
        ...
        let result = match cpu {
            ...
            CpuData::Int64(arr) => {
                // Approximate exp with GoldenExact
                CpuData::GoldenExact(arr.mapv(|x| GoldenExact::nearest((x as f64).exp(), 1 << 30)))
                // TODO: implement exact exp
            }
            CpuData::GoldenExact(arr) => {
                // Approximate transcendental function with GoldenExact
                CpuData::GoldenExact(arr.mapv(|x| GoldenExact::nearest(x.to_f64().exp(), 1 << 30)))
                // TODO: implement exact exp
            }
            CpuData::Rational(arr) => {
                CpuData::GoldenExact(arr.mapv(|x| GoldenExact::nearest(x.to_f64().exp(), 1 << 30)))
    ...
    pub fn exp_golden(&self) -> PyResult<TensorStorage> {
        ...
        let cpu = self.ensure_cpu()?;
        let result = match cpu {
            CpuData::Int64(arr) => CpuData::GoldenExact(arr.mapv(|x| GoldenExact::nearest((-(x as f64) * PHI_INV).exp(), 1 << 30))), // TODO: implement exact exp_golden
            CpuData::Complex128(_) => {
                return Err(PyErr::new::<pyo3::exceptions::PyTypeError, _>(
            ...
            CpuData::GoldenExact(arr) => {
                // Approximate golden exponential using φ̂ = 1/φ
                let phi_hat = GoldenExact::phi_hat();
                CpuData::GoldenExact(arr.mapv(|x| GoldenExact::nearest((-(x * phi_hat)).to_f64().exp(), 1 << 30)))
                // TODO: implement exact golden exponential
            }
            CpuData::Rational(arr) => {
                CpuData::GoldenExact(arr.mapv(|x| GoldenExact::nearest((-(x.to_f64()) * PHI_INV).exp(), 1 << 30)))
                // TODO: implement exact golden exponential
            }
            CpuData::FixedPoint64(arr) => {
    ...
    pub fn log(&self) -> PyResult<TensorStorage> {
        ...
        let result = match cpu {
            ...
            CpuData::Int64(arr) => CpuData::GoldenExact(arr.mapv(|x| GoldenExact::nearest((x as f64).ln(), 1 << 30))), // TODO: implement exact ln
            CpuData::GoldenExact(arr) => {
                CpuData::GoldenExact(arr.mapv(|x| GoldenExact::nearest(x.to_f64().ln(), 1 << 30)))
                // TODO: implement exact logarithm
            }
            CpuData::Rational(arr) => {
                CpuData::GoldenExact(arr.mapv(|x| GoldenExact::nearest(x.to_f64().ln(), 1 << 30)))
                // TODO: implement exact logarithm
            ...
            CpuData::FixedPoint64(arr) => {
                // log not implemented in fixed-point, approximate with GoldenExact
                CpuData::GoldenExact(arr.mapv(|x| GoldenExact::nearest((( x as f64) / (1i64 << 32) as f64).ln(), 1 << 30)))
                // TODO: implement exact logarithm
            }
            CpuData::Syntonic(_) => {
                return Err(PyErr::new::<pyo3::exceptions::PyTypeError, _>(
    ...
    pub fn sin(&self) -> PyResult<TensorStorage> {
        ...
        let result = match cpu {
            ...
            CpuData::Int64(arr) => CpuData::GoldenExact(arr.mapv(|x| GoldenExact::nearest((x as f64).sin(), 1 << 30))), // TODO: implement exact sin
            CpuData::GoldenExact(arr) => {
                CpuData::GoldenExact(arr.mapv(|x| GoldenExact::nearest(x.to_f64().sin(), 1 << 30)))
                // TODO: implement exact sin
            }
            CpuData::Rational(arr) => {
                CpuData::GoldenExact(arr.mapv(|x| GoldenExact::nearest(x.to_f64().sin(), 1 << 30)))
                // TODO: implement exact sin
    ...
    pub fn cos(&self) -> PyResult<TensorStorage> {
        ...
        let result = match cpu {
            ...
            CpuData::Int64(arr) => CpuData::GoldenExact(arr.mapv(|x| GoldenExact::nearest((x as f64).cos(), 1 << 30))), // TODO: implement exact cos
            CpuData::GoldenExact(arr) => {
                CpuData::GoldenExact(arr.mapv(|x| GoldenExact::nearest(x.to_f64().cos(), 1 << 30)))
                // TODO: implement exact cos
            }
            CpuData::Rational(arr) => {
                CpuData::GoldenExact(arr.mapv(|x| GoldenExact::nearest(x.to_f64().cos(), 1 << 30)))
                // TODO: implement exact cos
    ...
    pub fn sqrt(&self) -> PyResult<TensorStorage> {
        ...
        let result = match cpu {
            ...
            CpuData::Int64(arr) => CpuData::GoldenExact(arr.mapv(|x| GoldenExact::nearest((x as f64).sqrt(), 1 << 30))), // TODO: implement exact sqrt
            CpuData::GoldenExact(arr) => {
                CpuData::GoldenExact(arr.mapv(|x| GoldenExact::nearest(x.to_f64().sqrt(), 1 << 30)))
                // TODO: implement exact sqrt
            }
            CpuData::Rational(arr) => {
                CpuData::GoldenExact(arr.mapv(|x| GoldenExact::nearest(x.to_f64().sqrt(), 1 << 30)))
                // TODO: implement exact sqrt
    ...
    pub fn tanh(&self) -> PyResult<TensorStorage> {
        ...
        let result = match cpu {
            ...
            CpuData::Int64(arr) => CpuData::GoldenExact(arr.mapv(|x| GoldenExact::nearest((x as f64).tanh(), 1 << 30))), // TODO: implement exact tanh
            CpuData::GoldenExact(arr) => {
                CpuData::GoldenExact(arr.mapv(|x| GoldenExact::nearest(x.to_f64().tanh(), 1 << 30)))
                // TODO: implement exact tanh
            }
            CpuData::Rational(arr) => {
                CpuData::GoldenExact(arr.mapv(|x| GoldenExact::nearest(x.to_f64().tanh(), 1 << 30)))
                // TODO: implement exact tanh
            ...
            CpuData::FixedPoint64(arr) => {
                // tanh not implemented in fixed-point, approximate with GoldenExact
                CpuData::GoldenExact(arr.mapv(|x| GoldenExact::nearest((( x as f64) / (1i64 << 32) as f64).tanh(), 1 << 30)))
                // TODO: implement exact tanh
            }
            CpuData::Syntonic(_) => {
                return Err(PyErr::new::<pyo3::exceptions::PyTypeError, _>(
    ...
    pub fn sigmoid(&self) -> PyResult<TensorStorage> {
        ...
        let result = match cpu {
            ...
            CpuData::Int64(arr) => CpuData::GoldenExact(arr.mapv(|x| {
                let x_f64 = x as f64;
                GoldenExact::nearest(1.0 / (1.0 + (-x_f64).exp()), 1 << 30)
            })), // TODO: implement exact sigmoid
            CpuData::GoldenExact(arr) => {
                CpuData::GoldenExact(arr.mapv(|x| {
                    let x_f64 = x.to_f64();
                    GoldenExact::nearest(1.0 / (1.0 + (-x_f64).exp()), 1 << 30)
                }))
                // TODO: implement exact sigmoid
    ...
    pub fn eigh(&self) -> PyResult<(TensorStorage, TensorStorage)> {
        ...
        match cpu {
            CpuData::Complex128(arr) => {
                ...
                Ok((
                    Self::wrap_cpu(CpuData::Complex128(e.mapv(|x| Complex64::new(x, 0.0)).into_dyn()), &self.device),
                    Self::wrap_cpu(CpuData::Complex128(v.into_dyn()), &self.device),
                ))
            }
            _ => Err(PyErr::new::<pyo3::exceptions::PyNotImplementedError, _>(
        ...
        match cpu {
            CpuData::Complex128(arr) => {
                ...
                Ok((
                    ...
                    Self::wrap_cpu(CpuData::Complex128(s.mapv(|x| Complex64::new(x, 0.0)).into_dyn()), &self.device),
    ...
    pub fn compute_syntony_basic(&self) -> f64 {
        let epsilon = Rational::new(1, 10_000_000_000i128);
        let alpha_0 = Rational::new(1, 10);

        let d_psi = match self.differentiate(alpha_0.eval_f64()) {
            Ok(s) => s,
            Err(_) => return 0.5,
        };
    ...
    pub fn sum_reduce(&self, axis: Option<i32>, keepdim: bool) -> PyResult<TensorStorage> {
        ...
        if axis.is_none() {
            ...
            return Ok(TensorStorage {
                data: TensorData::Cpu(CpuData::GoldenExact(ArrayD::from_elem(IxDyn(&shape), GoldenExact::from_f64(sum)))),
                shape,
                device: self.device.clone(),
            });
        }
    ...
    pub fn mean_reduce(&self, axis: Option<i32>, keepdim: bool) -> PyResult<TensorStorage> {
        ...
        if axis.is_none() {
            ...
            let shape = if keepdim { vec![1] } else { vec![1] };
            return Ok(TensorStorage {
                data: TensorData::Cpu(CpuData::GoldenExact(ArrayD::from_elem(IxDyn(&shape), GoldenExact::from_f64(mean)))),
                shape,
                device: self.device.clone(),
            });
        }
...
impl TensorStorage {
    ...
    fn unary_cpu_fallback(&self, op: &str) -> PyResult<TensorStorage> {
        ...
        let result = match (op, &cpu_data) {
            // Float32/Float64 exp operations removed - use exact types only
            ("exp", CpuData::Complex128(arr)) => CpuData::Complex128(arr.mapv(|c| {
                let exp_re = c.re.exp();
                Complex64::new(exp_re * c.im.cos(), exp_re * c.im.sin())
            })),
            ("exp", CpuData::Int64(arr)) => CpuData::GoldenExact(arr.mapv(|x| GoldenExact::nearest((x as f64).exp(), 1 << 30))), // TODO: implement exact exp

            // Float32/Float64 log operations removed - use exact types only
            ("log", CpuData::Complex128(arr)) => CpuData::Complex128(arr.mapv(|c| c.ln())),
            ("log", CpuData::Int64(arr)) => CpuData::GoldenExact(arr.mapv(|x| GoldenExact::nearest((x as f64).ln(), 1 << 30))), // TODO: implement exact log

            // Float32/Float64 sin operations removed - use exact types only
            ("sin", CpuData::Complex128(arr)) => CpuData::Complex128(arr.mapv(|c| c.sin())),
            ("sin", CpuData::Int64(arr)) => CpuData::GoldenExact(arr.mapv(|x| GoldenExact::nearest((x as f64).sin(), 1 << 30))), // TODO: implement exact sin

            // Float32/Float64 cos operations removed - use exact types only
            ("cos", CpuData::Complex128(arr)) => CpuData::Complex128(arr.mapv(|c| c.cos())),
            ("cos", CpuData::Int64(arr)) => CpuData::GoldenExact(arr.mapv(|x| GoldenExact::nearest((x as f64).cos(), 1 << 30))), // TODO: implement exact cos

            // Float32/Float64 sqrt operations removed - use exact types only
            ("sqrt", CpuData::Complex128(arr)) => CpuData::Complex128(arr.mapv(|c| c.sqrt())),
            ("sqrt", CpuData::Int64(arr)) => CpuData::GoldenExact(arr.mapv(|x| GoldenExact::nearest((x as f64).sqrt(), 1 << 30))), // TODO: implement exact sqrt

            // Float32/Float64 tanh operations removed - use exact types only
            ("tanh", CpuData::Complex128(arr)) => CpuData::Complex128(arr.mapv(|c| c.tanh())),
            ("tanh", CpuData::Int64(arr)) => CpuData::GoldenExact(arr.mapv(|x| GoldenExact::nearest((x as f64).tanh(), 1 << 30))), // TODO: implement exact tanh

            // Float32/Float64 sigmoid operations removed - use exact types only
            ("sigmoid", CpuData::Complex128(arr)) => CpuData::Complex128(arr.mapv(|c| {
                let exp_neg_c = (-c).exp();
                Complex64::new(1.0, 0.0) / (Complex64::new(1.0, 0.0) + exp_neg_c)
            })),
            ("sigmoid", CpuData::Int64(arr)) => CpuData::GoldenExact(arr.mapv(|x| {
                let x_f64 = x as f64;
                GoldenExact::nearest(1.0 / (1.0 + (-x_f64).exp()), 1 << 30)
            })), // TODO: implement exact sigmoid

            // Float32/Float64 relu operations removed - use exact types only
            ...
            // Float32/Float64 neg operations removed - use exact types only
            ("neg", CpuData::Complex128(arr)) => CpuData::Complex128(arr.mapv(|c| -c)),
            ("neg", CpuData::Int64(arr)) => CpuData::Int64(arr.mapv(|x| -x)),

            // Float32/Float64 abs operations removed - use exact types only
            ("abs", CpuData::Complex128(arr)) => CpuData::Complex128(arr.mapv(|x| Complex64::new(x.norm(), 0.0))),
    ...
    fn binary_cpu_fallback(
        ...
        let result = match (op, &a_cpu, &b_cpu) {
            // Float32/Float64 add operations removed - use exact types only
            ...
            // Float32/Float64 sub operations removed - use exact types only
            ...
            // Float32/Float64 mul operations removed - use exact types only
            ...
            // Float32/Float64 div operations removed - use exact types only
...
pub fn srt_golden_gaussian_weights(vectors: &TensorStorage) -> PyResult<TensorStorage> {
    ...
    match &vectors.data {
        TensorData::Cpu(cpu_data) => match cpu_data {
            CpuData::Float64(arr) => {
                let mut weights = vec![Rational::from_integer(0).eval_f64(); count];
...
pub fn srt_e8_batch_projection(
    ...
    let proj_norm = {
        let two = GoldenExact::from_integer(2);
        let phi_plus_one = GoldenExact::phi().add(&GoldenExact::from_integer(1));
        let denominator = two.mul(&phi_plus_one);
        GoldenExact::from_integer(1).div(&denominator.sqrt()).eval_f64()
    };
        ...
        TensorData::Cpu(cpu_data) => {
            match cpu_data {
                CpuData::Float64(arr) => {
                    ...
                    let mut proj_parallel_data = vec![Rational::from_integer(0).eval_f64(); count * 4];
                    let mut proj_perp_data = vec![Rational::from_integer(0).eval_f64(); count * 4];
                    let mut q_values_data = vec![Rational::from_integer(0).eval_f64(); count];
            ...
            match (psi_cpu, norm_cpu) {
                (CpuData::Complex128(psi_arr), CpuData::Float64(norm_arr)) => {
                    ...
                    if denominator.lt(&epsilon) {
                        ...
                        Ok(numerator.div(&denominator).eval_f64())
                    }
                }
                _ => Err(PyErr::new::<pyo3::exceptions::PyTypeError, _>(
        ...
        (TensorData::Cpu(psi_cpu), TensorData::Cpu(norm_cpu)) => {
            ...
            match (psi_cpu, norm_cpu) {
                (CpuData::Complex128(psi_arr), CpuData::Float64(norm_arr)) => {
                    ...
                    let mut new_num = Rational::from_integer(0);
                    let mut new_den = Rational::from_integer(0);
                    ...
                    for (psi_val, &norm_sq) in psi_arr.iter().zip(norm_arr.iter()) {
                        ...
                        new_num = new_num.add(&Rational::from_f64_approx(new_amp_sq * golden_weight));
                        new_den = new_den.add(&Rational::from_f64_approx(new_amp_sq));
                    }

                    let epsilon = Rational::new(1, 1_000_000_000_000_000i128); // 1e-15
                    let new_syntony = if new_den.lt(&epsilon) {
                        syntony
                    } else {
                        new_num.div(&new_den).eval_f64()
                    };
