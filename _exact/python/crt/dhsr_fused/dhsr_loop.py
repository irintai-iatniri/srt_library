...
from applications.core.constants import SYNTONY_THRESHOLD
from applications.exact import PHI_INVERSE
from applications._core import SymExpr
from applications.crt.operators.mobius import apply_mobius_mask
from applications.nn.resonant_tensor import ResonantTensor

# Exact constants
PHI_INV_EXACT = PHI_INVERSE  # Exact GoldenExact
PHI_INV_SQ_EXACT = PHI_INVERSE * PHI_INVERSE  # Exact φ⁻²

# Numeric versions for compatibility (COMPAT: float evaluation for legacy code)
PHI_INV = PHI_INV_EXACT.eval()  # COMPAT: float evaluation for legacy code
PHI_INV_SQ = PHI_INV_SQ_EXACT.eval()  # COMPAT: float evaluation for legacy code
...
def evolve_state(
    ...
    while syntony < syntony_threshold and depth < max_recursion:
        ...
        alpha = (1.0 - syntony) * PHI_INV_SQ  # phi^-2 ≈ 0.382
...
def compute_optimal_strength(syntony: float) -> float:
    ...
    return PHI_INV + syntony * PHI_INV_SQ
...
class DHSRLoop:
    ...
    def step(self) -> Tuple[ResonantTensor, float, bool]:
        ...
        # Single cycle
        new_state, new_syntony = single_dhsr_cycle(self.current_state, alpha, strength)
        ...
        is_crystallized = (
            new_syntony >= self.syntony_threshold or self.depth >= self.max_recursion
        )
