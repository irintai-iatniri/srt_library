...
from applications.core.dtype import complex128
from applications.core.state import State
from applications.exact import PHI_NUMERIC, Q_DEFICIT_NUMERIC, PHI, PHI_INVERSE
from applications._core import SymExpr, GoldenExact, Rational
from applications.srt.geometry import (
    WindingState,
)
from applications.srt.spectral import (
    theta_series,
)
...
PHI_SYMEXPR = SymExpr.phi()  # Golden ratio φ
PHI_INV_SYMEXPR = SymExpr.from_int(1) / SymExpr.phi()  # φ⁻¹
PHI_INV_SQ_SYMEXPR = PHI_INV_SYMEXPR * PHI_INV_SYMEXPR  # φ⁻²
Q_DEFICIT_SYMEXPR = SymExpr.q()  # Universal syntony deficit

# Numeric constants for computation compatibility (COMPAT: float evaluation for legacy code)
PHI_SYMEXPR_NUM = PHI_SYMEXPR.eval()  # COMPAT: float evaluation for legacy code
PHI_INV_NUM = PHI_INV_SYMEXPR.eval()  # COMPAT: float evaluation for legacy code  
PHI_INV_SQ_NUM = PHI_INV_SQ_SYMEXPR.eval()  # COMPAT: float evaluation for legacy code
Q_DEFICIT_NUM = Q_DEFICIT_SYMEXPR.eval()  # COMPAT: float evaluation for legacy code

# Backward compatibility aliases
PHI_LOCAL = PHI_SYMEXPR_NUM
PHI_INV = PHI_INV_NUM
PHI_INV_SQ = PHI_INV_SQ_NUM
Q_DEFICIT = Q_DEFICIT_NUM

# Verify golden partition: D + H = 1
assert abs(PHI_INV + PHI_INV_SQ - 1) < 1e-10, "Golden partition violated!"  # eval boundary
...
def windings_to_mode_norms(windings: List[WindingState]) -> List[float]:
    ...
    """
    return [float(w.norm_squared) for w in windings]  # export boundary
...
def golden_distribution(N: int) -> List[float]:
    ...
    # Use exact phi value for computation
    phi_value = PHI_SYMEXPR.eval()
    weights = [math.exp(-n * n / phi_value) for n in n_values]
    total = sum(weights)
...
def harmonize(
    psi: State, strength: float = None, preserve_phase: bool = True
) -> State:
    """
    ...
    """
    if strength is None:
        strength = PHI_INV_SYMEXPR.eval()  # Use exact φ⁻¹
    ...
    if preserve_phase:
        values = psi.to_list()
        phases = [
            cmath.phase(v) if abs(v) > 1e-15 else random.uniform(0, 2 * math.pi)  # eval boundary
            for v in values
        ]
    else:
        phases = [0] * N
...
def differentiate(
    psi: State, syntony: float = Rational(1, 2), alpha_0: float = Rational(1, 10), seed: Optional[int] = None
) -> State:
    """
    ...
    alpha = alpha_0 * (1 - syntony)  # eval boundary
    ...
    theta = theta_series(phi=PHI_SYMEXPR.eval(), max_norm=int(math.sqrt(N)))
    theta_value = theta.evaluate(1)  # Get theta series value
    ...
    for i in range(N):
        ...
        boost_factor = 1 + alpha * math.sqrt(mode_norms[i % len(mode_norms)])  # eval boundary
...
def compute_syntony(
    ...
    # Clamp using exact values
    zero = 0  # Use exact integer
    two = 2   # Use exact integer
    return min(max(S, zero), two)  # Allow slightly above 1 during transients
...
class DHSRTrajectory:
    ...
    def final_syntony(self) -> float:
        return self.syntony_values[-1] if self.syntony_values else 0
...
def verify_implementation():
    ...
    H_psi = harmonize(psi, strength=PHI_INV_SYMEXPR.eval())
    golden_target = golden_distribution(N)
    total_energy = sum(abs(v) ** 2 for v in psi.to_list())
    target_amplitudes = [math.sqrt(total_energy * w) for w in golden_target]
    ...
    phi_inv_target = PHI_INV_SYMEXPR.eval()
    print(f"Target (φ⁻¹):    {phi_inv_target:.6f}")
    deviation = abs(traj.syntony_values[-1] - phi_inv_target)
    ...
    phi_inv_exact = PHI_INV_SYMEXPR.eval()
    agreement = (1 - abs(mean_S - phi_inv_exact) / phi_inv_exact) * 100
...
if __name__ == "__main__":
    ...
    for i in range(50):
        ...
        if i < 15 or i % 10 == 0:
            phi_inv_exact = PHI_INV_SYMEXPR.eval()
            delta = S - phi_inv_exact
            print(f"Step {i:2d}: S = {S:.6f}, Δ = {delta:+.6f}")
        current = recurse(current, syntony=S)

    S_final = compute_syntony(current)
    phi_inv_exact = PHI_INV_SYMEXPR.eval()
    print(f"Final: S = {S_final:.6f}")
    print(f"Target (φ⁻¹): {phi_inv_exact:.6f}")
    agreement = (1 - abs(S_final - phi_inv_exact) / phi_inv_exact) * 100
    print(".2f")
