...
def compute_mobius_mask(size: int) -> List[int]:
    """
    Compute the Mobius mask for a tensor of given size.

    Dimensions where mu(n+1) = 0 are masked (set to 0).
    Other dimensions are preserved (set to 1).

    Args:
        size: Number of elements

    Returns:
        List of mask values (1 or 0) as exact integers
    """
    mask = []
    for i in range(size):
        # Use 1-indexed dimension numbers (dimension 1, 2, 3, ...)
        mu = mobius(i + 1)
        mask.append(1 if mu != 0 else 0)  # Use exact integer values
    return mask
