...
Uses exact golden ratio arithmetic from syntonic_applications.exact.GoldenExact.

Example:
    >>> from syntonic_applications.srt.golden import golden_recursion
    >>> from syntonic_applications.srt.geometry import winding_state
    >>> R = golden_recursion()
...
from applications.exact import PHI, GoldenExact
from applications.srt.geometry.winding import WindingState, enumerate_windings
...
class GoldenRecursionMap:
    ...
    Uses exact golden ratio φ from syntonic_applications.exact.GoldenExact.
    ...
    def apply_inverse_approx(self, n: WindingState) -> WindingState:
        ...
        """
        inv_phi = self._phi - 1  # 1/φ = φ - 1 (exact golden ratio property)
        return WindingState(
    ...
    def mass_scaling(self, n: WindingState, max_depth: int = 100) -> float:
        ...
        if n.is_zero():
            return 1  # Use exact integer
        k = self.orbit_depth(n, max_depth)
        if k < 0:
            return 0  # No convergence - exact integer
