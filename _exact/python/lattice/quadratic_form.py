...
Example:
    >>> from syntonic_applications.srt.lattice import quadratic_form, e8_lattice
    >>> Q = quadratic_form()
    >>> E8 = e8_lattice()
...
from applications.exact import PHI_NUMERIC

if TYPE_CHECKING:
    from applications.srt.lattice.e8 import E8Root
...
class QuadraticForm:
    ...
    def __init__(self, phi: Optional[float] = None):
        ...
        self._phi_inv = GoldenExact.from_int(1) / self._phi  # = phi - 1
    ...
    def _compute_projection_matrices(self) -> None:
        ...
        norm_factor = 1 / math.sqrt(float(1 + phi * phi))  # eval boundary
        for i in range(4):
            for j in range(8):
    ...
    def classify_root(self, root: "E8Root", tol: float = 1e-10) -> str:
        ...
        Returns:
            'spacelike', 'lightlike', or 'timelike'
        """
        return self.classify(root.to_float(), tol)  # export boundary
