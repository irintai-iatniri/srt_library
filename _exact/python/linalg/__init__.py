...
from applications.core.dtype import complex128, float64
from applications.core.state import State
from applications.exceptions import LinAlgError
...
def dot(a: State, b: State) -> float:
    ...
    if a.dtype.name == "complex128" or b.dtype.name == "complex128":
        ...
        return result
    # Keep exact type - don't convert to float
...
def pinv(a: State) -> State:
    ...
    s_flat = s.to_list()
    tol = max(a.shape) * max(abs(x) for x in s_flat) * 1e-15  # Keep for tolerance check
    s_inv = [1 / x if abs(x) > tol else 0 for x in s_flat]  # Use exact integer division

    # Build S^+ matrix (n x m) - transpose of diagonal matrix with inverted singular values
    s_inv_mat = [0] * (n * m)  # Use exact integer zeros
    for i in range(k):
...
def mm_add(
    a: State, b: State, c: State, alpha: float = 1, beta: float = 1  # Use exact integers as defaults
) -> State:
    """
...
def mm_corrected(a: State, b: State, structure: Structure, sign: int = 1) -> State:
    ...
    Example:
        >>> from syntonic_applications.linalg import mm_corrected, Structure
