...
import applications.sn as sn
from applications.nn.winding.prime_selection import PurePrimeSelectionLayer

from applications._core import SymExpr, GoldenExact, Rational
from applications.exact import GoldenExact

# Use exact golden ratio values
PHI = GoldenExact.phi()  # Exact golden ratio 
PHI_INV = GoldenExact(-1, 1)  # Exact φ^-1 = φ - 1
PHI_INV_SQ = GoldenExact(3, -2, 2, -1)  # Exact φ^-2
...
class ResonantWindingDHSRBlock(sn.Module):
    ...
    def __init__(
        ...
        consensus_threshold: float = Rational(3, 125),  # 0.024 = 3/125
        precision: int = 100,
        noise_scale: float = Rational(1, 100),
        dropout: float = Rational(1, 10),
    ):
        """
        ...
        self.last_d_duration = 0
        self.last_h_duration = 0
        self.last_ratio = 0
    ...
    def forward(
        ...
        mode_norms_full = (
            ...
            if mode_norms
            else [(i % self.dim) ** 2 for i in range(batch_size * self.dim)]  # Keep mode norms as exact integers
        )
        ...
        for b in range(batch_size):
            ...
            try:
                ...
                single_tensor = single_tensor.harmonize(PHI_INV.eval(), 0)  # Use display value for interface, exact zero
        ...
        syntony_new = (
            sum(batch_syntonies) / len(batch_syntonies) if batch_syntonies else Rational(1, 2)
        )
    ...
    def get_timing_stats(self) -> dict:
        ...
        return {
            ...
            "target_ratio": PHI.eval(),  # Use display value for interface
            "phi_dwell_satisfied": (
                abs(self.last_ratio - PHI.eval()) < Rational(1, 10) if self.last_ratio > 0 else False  # eval boundary
            ),
        }
...
if __name__ == "__main__":
    ...
    mode_norms = [i * i for i in range(batch_size * dim)]  # Keep as exact integers
    x = ResonantTensor(data, [batch_size, dim], mode_norms, 100)
    mode_norm_list = [i * i for i in range(dim)]  # Keep as exact integers
    prev_syntony = Rational(1, 2)
