...
Example:
    >>> from syntonic_applications.srt.spectral import heat_kernel
    >>> K = heat_kernel()
    >>> K.evaluate(1.0)
    ...
"""
...
from applications.exact import PHI
from applications.srt.geometry.winding import (
    WindingState,
    enumerate_windings_by_norm,
    winding_state,
)

# Base eigenvalue for standard Laplacian on W⁴: λₙ = 4π²|n|²
_FOUR_PI_SQ = 4 * SymExpr.pi() * SymExpr.pi()
_BASE_EIGENVALUE = _FOUR_PI_SQ.eval()  # eval boundary
...
class HeatKernel:
    ...
    The Laplacian eigenvalues on W⁴ are λₙ = 4π²|n|² for winding
    ...
    def __init__(
        ...
        Uses exact golden ratio φ from syntonic_applications.exact.PHI by default.
    ...
    def eigenvalue(self, n: WindingState) -> float:
        ...
        Returns:
            Eigenvalue λₙ
        """
        return _BASE_EIGENVALUE * n.norm_squared
    ...
    def short_time_expansion(self, t: float, terms: int = 3) -> float:
        ...
        """
        d = 4  # Dimension of W⁴
        four_pi = 4 * SymExpr.pi()
        prefactor = (four_pi.eval() * t) ** (-d / 2)  # eval boundary

        # For flat torus, a₀ = vol(W⁴) / (4π)^(d/2)
        # We normalize so a₀ = 1 at t = 0
        # Higher coefficients depend on curvature (zero for flat torus)
        a0 = 1  # Use exact integer for mathematical coefficient
        a1 = 0  # No curvature term for flat torus
        a2 = 0
    ...
    def weyl_law_check(self, lambda_max: float) -> Tuple[int, float, float]:
        ...
        d = 4
        sixteen_pi_sq = 16 * SymExpr.pi() * SymExpr.pi()
        weyl_coeff = 1 / sixteen_pi_sq.eval()  # eval boundary
