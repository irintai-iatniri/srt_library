...
fn main() {
    // Configure PyO3 for this build
    // This emits the necessary cargo config for linking
    pyo3_build_config::use_pyo3_cfgs();
    
    // For tests to work, we need to link against libpython
    // The extension-module feature prevents this, but we can override for test builds
    let profile = std::env::var("PROFILE").unwrap_or_default();
    if profile == "debug" || std::env::var("CARGO_CFG_TEST").is_ok() {
        // Emit cargo directives to link Python for tests
