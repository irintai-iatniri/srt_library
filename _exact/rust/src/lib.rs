...
#[pyfunction]
fn srt_pi() -> f64 {
    crate::constants::SRT_PI
}

/// Get e (Euler's number) constant
#[pyfunction]
fn srt_e() -> f64 {
    crate::constants::SRT_E
}
...
fn _core(m: &Bound<'_, PyModule>) -> PyResult<()> {
    ...
    m.add_function(wrap_pyfunction!(py_eye, m)?)?;
    m.add_function(wrap_pyfunction!(py_stack, m)?)?;
    m.add_function(wrap_pyfunction!(py_silu, m)?)?;
    m.add_function(wrap_pyfunction!(py_golden_silu, m)?)?;
    m.add_function(wrap_pyfunction!(py_where, m)?)?;
    m.add_function(wrap_pyfunction!(py_einsum, m)?)?;
    m.add_function(wrap_pyfunction!(py_linear, m)?)?;
    m.add_function(wrap_pyfunction!(py_pixel_shuffle, m)?)?;
