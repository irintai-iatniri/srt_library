...
use crate::tensor::nn_ops;

/// Create evenly spaced values in [start, end) with given step.
#[pyfunction]
pub fn py_arange(start: f64, end: f64, step: f64) -> Vec<f64> {
    nn_ops::arange(start, end, step)
}

/// Create linearly spaced values in [start, end].
#[pyfunction]
pub fn py_linspace(start: f64, end: f64, steps: usize) -> Vec<f64> {
    nn_ops::linspace(start, end, steps)
}

/// Create n×n identity matrix.
#[pyfunction]
pub fn py_eye(n: usize) -> (Vec<f64>, Vec<usize>) {
    let (data, shape) = nn_ops::eye(n);
    (data, shape.to_vec())
}

/// Stack tensors along a new dimension.
#[pyfunction]
pub fn py_stack(tensors: Vec<Vec<f64>>, shapes: Vec<Vec<usize>>, dim: usize) -> (Vec<f64>, Vec<usize>) {
    let tensor_refs: Vec<&[f64]> = tensors.iter().map(|t| t.as_slice()).collect();
    let shape_refs: Vec<&[usize]> = shapes.iter().map(|s| s.as_slice()).collect();
    nn_ops::stack(&tensor_refs, &shape_refs, dim)
}

/// SiLU activation: x * sigmoid(x).
#[pyfunction]
pub fn py_silu(input: Vec<f64>) -> Vec<f64> {
    nn_ops::silu(&input)
}

/// Golden SiLU: x * sigmoid(φ*x).
#[pyfunction]
pub fn py_golden_silu(input: Vec<f64>) -> Vec<f64> {
    nn_ops::golden_silu(&input)
}

/// Elementwise conditional: out[i] = x[i] if cond[i] > 0 else y[i].
#[pyfunction]
pub fn py_where(condition: Vec<f64>, x: Vec<f64>, y: Vec<f64>) -> Vec<f64> {
    nn_ops::where_select(&condition, &x, &y)
}

/// General einsum contraction.
#[pyfunction]
pub fn py_einsum(equation: String, operands: Vec<Vec<f64>>, shapes: Vec<Vec<usize>>) -> (Vec<f64>, Vec<usize>) {
    let refs: Vec<(&[f64], &[usize])> = operands
        .iter()
        .zip(shapes.iter())
        .map(|(d, s)| (d.as_slice(), s.as_slice()))
        .collect();
    nn_ops::einsum(&equation, &refs)
}
...
#[pyfunction]
pub fn py_pixel_shuffle(data: Vec<f64>, shape: Vec<usize>, upscale: usize) -> (Vec<f64>, Vec<usize>) {
    let s: [usize; 4] = [shape[0], shape[1], shape[2], shape[3]];
    let (output, out_shape) = nn_ops::pixel_shuffle(&data, &s, upscale);
    (output, out_shape.to_vec())
}

/// Upsample: nearest or bilinear.
#[pyfunction]
pub fn py_upsample(data: Vec<f64>, shape: Vec<usize>, scale: usize, mode: String) -> (Vec<f64>, Vec<usize>) {
    let s: [usize; 4] = [shape[0], shape[1], shape[2], shape[3]];
    let (output, out_shape) = match mode.as_str() {
        "bilinear" => nn_ops::upsample_bilinear(&data, &s, scale),
        _ => nn_ops::upsample_nearest(&data, &s, scale),
    };
    (output, out_shape.to_vec())
}
...
pub fn py_gru_cell(
    ...
    nn_ops::gru_cell(
        &x, &h, &w_ir, &w_hr, &b_r, &w_iz, &w_hz, &b_z, &w_in, &w_hn, &b_n,
        batch, input_size, hidden_size,
    )
}

/// LSTM cell: single timestep. Returns (h_new, c_new).
#[pyfunction]
pub fn py_lstm_cell(
    x: Vec<f64>,
    h: Vec<f64>,
