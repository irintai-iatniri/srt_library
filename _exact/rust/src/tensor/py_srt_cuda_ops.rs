...
// SRT Constants 2014 canonical imports
use crate::constants::{PHI, PHI_INV, Q_DEFICIT_CONST as Q_DEFICIT};
...
fn cpu_sin_toroidal_f64(data: &[f64]) -> Vec<f64> {
    let two_pi = 2.0 * crate::constants::SRT_PI;
...
fn cpu_atan2_toroidal_f64(y: &[f64], x: &[f64]) -> Vec<f64> {
    y.iter()
        .zip(x.iter())
        .map(|(&yi, &xi)| yi.atan2(xi) / (2.0 * crate::constants::SRT_PI))
        .collect()
}
...
pub fn py_phase_syntony_c128(input: Vec<(f64, f64)>, device_idx: usize) -> PyResult<f64> {
    ...
    let variance = phases.iter()
        .map(|phase| {
            ...
            let normalized_diff = if diff > crate::constants::SRT_PI {
                diff - 2.0 * crate::constants::SRT_PI
            } else if diff < -crate::constants::SRT_PI {
                diff + 2.0 * crate::constants::SRT_PI
            } else {
                diff
            };
