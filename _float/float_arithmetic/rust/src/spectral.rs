...
const PHI_FP: FixedPoint64 = FixedPoint64(6949826725); // 1.61803398875 * 2^32
const PI_FP: FixedPoint64 = FixedPoint64(13493037704); // 3.141592 * 2^32
...
pub fn knot_spectral_zeta_complex(
    ...
    for w in &windings {
        ...
        if n_sq > 0 {
            ...
            let normalized_arg = exp_arg_imag - (2.0 * PI) * (exp_arg_imag / (2.0 * PI)).round();

            let magnitude = exp_arg_real.exp();
            sum_real += magnitude * normalized_arg.cos();
            sum_imag += magnitude * normalized_arg.sin();
        }
    }
