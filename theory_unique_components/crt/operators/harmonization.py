...
from srt_library.core.exact import GoldenExact, Rational, PHI_INVERSE as PHI_INV
...
# Use exact golden inverse from srt_library.core.exact (for exact-first SGC)
...
class HarmonizationOperator(OperatorBase):
    ...
    def __init__(
        ...
        if damping_projectors is not None:
            ...
            self._dampers = create_damping_cascade(
                ...
                base_cutoff=0.7,
                decay=PHI_INV.eval() if hasattr(PHI_INV, 'eval') else PHI_INV,
            )
    ...
    def _decay_factor(self, level: int, delta_d: Optional[GoldenExact]) -> GoldenExact:
        ...
        return base_decay * GoldenExact.nearest(min(modulation.eval(), 2.0), 1 << 30)
        ...
        **kwargs,
        ...
        if delta_d is not None:
            if isinstance(delta_d, GoldenExact):
                ...
                delta_d_exact = GoldenExact.nearest(float(delta_d), 1 << 30)
    ...
    def harmonization_magnitude(self, state: "State") -> GoldenExact:
        ...
        return GoldenExact.nearest(norm_float, 1 << 30)
...
class SyntonicHarmonization:
    def __init__(self, damping_factor="PHI_INV"):
        ...
        """
        from srt_library.core.exact import PHI_INVERSE, Rational, GoldenExact
    ...
    def apply(self, state: "State") -> "State":
        ...
        from srt_library.core._core import TensorStorage
